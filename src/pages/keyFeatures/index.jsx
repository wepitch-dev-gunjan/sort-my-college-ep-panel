...
const KeyFeatures = () => {
  ...
  const { editKeyFeatureEnable, setEditKeyFeatureEnable } =
    ...
    async function fetchRemainingFeatures() {
      try {
        const { data } = await axios.get(
          `${backend_url}/admin/key-features-institute/remaining-key-features-for-institute`,
          {
            headers: {
              Authorization: user.token,
            },
  ...
  const handleSave = async () => {
    try {
      ...
      const new_key_features = keyFeaturesInstitute.map((feature) => ({
        _id: feature._id,
      }));

      await axios.put(
        `${backend_url}/ep/key-features/edit`,
        { new_key_features },
        {
          headers: {
            Authorization: user.token,
    ...
    <div className="key-features-parent">
      <div className="key-features-head">
        ...
        {!editKeyFeatureEnable ? (
          <button
            className="kf-edit"
            onClick={() => setEditKeyFeatureEnable(true)}
          >
            Edit
          </button>
        ) : (
          <div className="cancel-save-btns">
            <button className="kf-save" onClick={handleSave}>
              {loading ? <Spinner /> : "Save"}
            </button>
            <button
              className="kf-cancel"
              onClick={() => setEditKeyFeatureEnable(false)}
            >
              Cancel
            </button>
          </div>
      ...
      {editKeyFeatureEnable ? (
        <div className="edit-key-features">
          <div
            className="key-features-child key-features-remaining"
          ...
          <GoArrowSwitch />
          <div
            className="key-features-child key-features-existing"
